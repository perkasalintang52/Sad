...
let handler = async (m, { conn, isROwner, text }) => {
    ...
    for (let i of chats) {
    await delay(500)
    let message = await prepareWAMessageMedia({ video: fs.readFileSync('./src/menu.mp4'), gifPlayback: true }, { upload: conn.waUploadToServer })
     const template = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
     templateMessage: {
         hydratedTemplate: {
           videoMessage: message.videoMessage,
           hydratedContentText: text.trim(),
           hydratedFooterText: `Bot By romli 🗿`,
           hydratedButtons: [{
             urlButton: {
               displayText: 'Group ZxyuuRomm',
               url: gc
             }
